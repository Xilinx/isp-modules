...
static void my_tasklet_function(void  *data) {
    ...
    } else if (recv_cmd_id == MB_CMD_RES_SUCCESS || recv_cmd_id == MB_CMD_GET_SUCCESS) {
        complete(&mailbox_completion);
    } else if (recv_cmd_id ==  RPU_2_APU_CMD_DISPLAY_BUFFER) {
        pr_err("%s %d recv_cmd_id %d \n",__func__,__LINE__,recv_cmd_id);
...
static void vvcam_isp_mb_rx_cb(struct mbox_client *cl, void *msg)
{
    ...
    if (rpu != NULL) {
        pr_info("vvcam_isp_mb_rx_cb: Received interrupt from RPU ID %d\n", rpu_id);
...
static int vvcam_isp_setup_mbox(struct rpu_dev *rpu, struct device_node *node)
{
    ...
    dev_info(rpu->dev, "Mailbox setup completed successfully.\n");
    return 0;
}
...
static struct rpu_dev *find_or_create_rpu(struct platform_device *pdev, int rpu_id)
{
    ...
    dev_info(&pdev->dev, "Initialized RPU with ID %d\n", rpu_id);
...
void xlnx_mbox_apu_wait_for_ack(struct vvcam_isp_dev *isp_dev)
{
    ...
    dev_info(isp_dev->dev, "Acknowledgment received successfully.\n");
}
