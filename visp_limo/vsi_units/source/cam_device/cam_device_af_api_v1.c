...
RESULT VsiCamDeviceAfRegister(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_REGISTER, &packet,
...
RESULT VsiCamDeviceAfUnRegister(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_UNREGISTER, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
							 ...
							 CamDeviceAfMode_t *pMode)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_SET_MODE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
							 ...
							 CamDeviceAfMode_t *pMode)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_GET_MODE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
							   ...
							   CamDeviceAfConfig_t *pConfig)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_SET_CONFIG, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
							   ...
							   CamDeviceAfConfig_t *pConfig)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_GET_CONFIG, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfSetRoiIndex(CamDeviceHandle_t hCamDevice, uint8_t roiIndex)
{
	RESULT result = RET_SUCCESS;
	int ret = 0;
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_SET_ROI_INDEX, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
								 ...
								 uint8_t *pRoiIndex)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_GET_ROI_INDEX, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfEnable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_ENABLE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfDisable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_DISABLE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
							   ...
							   CamDeviceAfStatus_t *pStatus)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_GET_STATUS, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
								...
								uint32_t *pVersion)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_GET_VERSION, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfReset(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AF_RESET, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
