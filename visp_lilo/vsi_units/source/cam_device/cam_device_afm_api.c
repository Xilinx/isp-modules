...
)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_SET_THRESHOLD, &packet,
								   ...
								   uint32_t *pThreshold)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_GET_THRESHOLD, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
									...
									CamDeviceAfmMeasureResult_t *pResult)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_GET_STATISTICS, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
									   ...
									   CamDeviceAfmWindowConfig_t *pWindow)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_SET_MEASURE_WINDOW, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
									   ...
									   CamDeviceAfmWindowConfig_t *pWindow)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_GET_MEASURE_WINDOW, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfmEnable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_ENABLE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfmDisable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_DISABLE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
								...
								CamDeviceAfmStatus_t *pStatus)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_GET_STATUS, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
								 ...
								 uint32_t *pVersion)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_GET_VERSION, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDeviceAfmReset(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_AFM_RESET, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
