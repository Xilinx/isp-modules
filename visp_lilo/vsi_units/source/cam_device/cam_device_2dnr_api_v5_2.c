...
RESULT VsiCamDevice2DnrEnable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_2DNR_ENABLE, &packet,
...
RESULT VsiCamDevice2DnrDisable(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_2DNR_DISABLE, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
...
RESULT VsiCamDevice2DnrReset(CamDeviceHandle_t hCamDevice)
{
	...
	ret = Send_Command(APU_2_RPU_MB_CMD_2DNR_RESET, &packet,
					   packet.payload_size + payload_extra_size, dest_cpu_id,
					   src_cpu_id);
	if (0 != ret)
	{
