...
#define ALOGV(...)                                \
	if (isp_log_level() >= ISP_LOG_LEVEL_VERBOSE) \
	__android_log_print(                          \
		ANDROID_LOG_INFO, ISP_INFO,               \
		"\033[1;30;37mVERBOSE : " __ALOG_INT(__VA_ARGS__, "\n")) // white

#define ALOGD(...)                              \
	if (isp_log_level() >= ISP_LOG_LEVEL_DEBUG) \
	__android_log_print(                        \
		ANDROID_LOG_INFO, ISP_INFO,             \
		"\033[1;30;37mDEBUG  : " __ALOG_INT(__VA_ARGS__, "\n")) // white

#define ALOGI(...)                             \
	if (isp_log_level() >= ISP_LOG_LEVEL_INFO) \
	__android_log_print(                       \
		ANDROID_LOG_INFO, ISP_INFO,            \
		"\033[1;30;32mINFO   : " __ALOG_INT(__VA_ARGS__, "\n")) // green

#define ALOGW(...)                                \
	if (isp_log_level() >= ISP_LOG_LEVEL_WARNING) \
	__android_log_print(                          \
		ANDROID_LOG_INFO, ISP_INFO,               \
		"\033[1;30;33mWARN   : " __ALOG_INT(__VA_ARGS__, "\n")) // yellow

#define ALOGE(...)                              \
	if (isp_log_level() >= ISP_LOG_LEVEL_ERROR) \
	__android_log_print(                        \
		ANDROID_LOG_INFO, ISP_INFO,             \
		"\033[1;30;31mERROR  : " __ALOG_INT(__VA_ARGS__, "\n")) // red

#else

#define ALOGV(...)                                \
	if (isp_log_level() >= ISP_LOG_LEVEL_VERBOSE) \
	printf("\033[1;30;37mVERBOSE : " __ALOG_INT(__VA_ARGS__, "\n")) // white

#define ALOGD(...)                              \
	if (isp_log_level() >= ISP_LOG_LEVEL_DEBUG) \
	printf("\033[1;30;37mDEBUG  : " __ALOG_INT(__VA_ARGS__, "\n")) // white

#define ALOGI(...)                             \
	if (isp_log_level() >= ISP_LOG_LEVEL_INFO) \
	printf("\033[1;30;32mINFO   : " __ALOG_INT(__VA_ARGS__, "\n")) // green

#define ALOGW(...)                                \
	if (isp_log_level() >= ISP_LOG_LEVEL_WARNING) \
	printf("\033[1;30;33mWARN   : " __ALOG_INT(__VA_ARGS__, "\n")) // yellow

#define ALOGE(...)                              \
	if (isp_log_level() >= ISP_LOG_LEVEL_ERROR) \
	printf("\033[1;30;31mERROR  : " __ALOG_INT(__VA_ARGS__, "\n")) // red
