...
RESULT VsiCamDeviceDgSetConfig(CamDeviceHandle_t hCamDevice,
...
RESULT VsiCamDeviceDgGetConfig(CamDeviceHandle_t hCamDevice,
							   CamDeviceDgConfig_t *pDgCfg)
{
	RESULT result = RET_SUCCESS;
...
RESULT VsiCamDeviceDgEnable(CamDeviceHandle_t hCamDevice)
{
	RESULT result = RET_SUCCESS;
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DG_ENABLE, &packet,
						  packet.payload_size + payload_extra_size, dest_cpu_id,
						  src_cpu_id);
	if (0 != result)
	{
...
RESULT VsiCamDeviceDgDisable(CamDeviceHandle_t hCamDevice)
{
	RESULT result = RET_SUCCESS;
...
RESULT VsiCamDeviceDgGetStatus(CamDeviceHandle_t hCamDevice,
...
RESULT VsiCamDeviceDgReset(CamDeviceHandle_t hCamDevice)
{
	RESULT result = RET_SUCCESS;
	...
	result = Send_Command(APU_2_RPU_MB_CMD_DG_RESET, &packet,
						  packet.payload_size + payload_extra_size, dest_cpu_id,
						  src_cpu_id);
	if (0 != result)
	{
...
RESULT VsiCamDeviceDgGetVersion(CamDeviceHandle_t hCamDevice,
								uint32_t *pVersion)
{
	RESULT result = RET_SUCCESS;
