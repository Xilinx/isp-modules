...
//    pr_err("%s-%d (((uint64_t *)fifo->buffer) + fifo->write_offset) = 0x%llx\n",__func__,__LINE__,(((uint64_t *)fifo->buffer) + fifo->write_offset));	pr_	
 //   fifo_info(fifo);
    //memcpy((((uint64_t)fifo->buffer_virt) + fifo->write_offset), msg, fifo->item_size);
...
int fifo_read(MboxPostMsg *msg, FifoControl *fifo)
{
  ...
  //  xil_printf("fifo_read reading from -> %x \n ",(0xFFFFFFF & (unsigned int)fifo->buffer) + fifo->read_offset);

    //memcpy(msg, ((fifo->buffer_virt) + (fifo->read_offset)), fifo->item_size);
