...
)
{
    ...
    ret = Send_Command (APU_2_RPU_MB_CMD_UNREGISTER_AWB_LIB, &packet, packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
    if(0 != ret) {
        os_unlock_mutex(&mbox_lock);
        return RET_FAILURE;
    }
...
)
{
    ...
    ret = Send_Command (APU_2_RPU_MB_CMD_AWB_DISABLE, &packet, packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
    if(0 != ret) {
        os_unlock_mutex(&mbox_lock);
        return RET_FAILURE;
    }
