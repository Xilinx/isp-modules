...
(
    ...
    result = Send_Command (APU_2_RPU_MB_CMD_FLEXA_SBI_MI_SET_CONFIG, &packet, packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
    if(0 != result) {
...
(
    ...
    result = Send_Command (APU_2_RPU_MB_CMD_FLEXA_SBI_MI_GET_CONFIG, &packet, packet.payload_size + payload_extra_size, dest_cpu_id, src_cpu_id);
    if(0 != result) {
        os_unlock_mutex(&mbox_lock);
        return RET_FAILURE;
    }
