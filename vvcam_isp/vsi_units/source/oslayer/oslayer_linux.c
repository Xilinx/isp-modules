 ...
 * parameter.
 */
__attribute__((unused)) static inline int64_t timespec_to_ns(const struct timespec *ts)
{
...
int32_t osTimeStampNs(float32_t *pTimeStamp)
{
    ...
    timeTmp = (((uint64_t)tp.tv_sec * OS_USEC_PER_SEC)
            + ((uint64_t)tp.tv_nsec / (OS_NSEC_PER_SEC / OS_USEC_PER_SEC)));
    *pTimeStamp = (float32_t)timeTmp / (float32_t)OS_USEC_PER_SEC;
#endif
    return (OSLAYER_OK);
}
...
int32_t osEventTimedWait(osEvent *pEvent, uint32_t msec)
{
    ...
    if(!pEvent->state)
    {
        ...
        gettimeofday(&tval, NULL);
        usec = (uint64_t)1000 * msec + tval.tv_usec;
