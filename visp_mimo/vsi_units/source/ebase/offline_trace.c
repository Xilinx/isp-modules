...
void OpenOffLineTrace(
    Offline_Tracer *off_Tracer,
    uint32_t instanceId,
    const char *path_suffix)
{
    ...
    sprintf(file_path, "%s/%sinstance%u.log", temp_prefix, path_suffix, instanceId);
    fprintf(stdout,"%s:%s\n",__func__,file_path);
    ...
    const char* sFormat, ...)
{
    ...
    if (off_Tracer->enabled != 0)
    {
        ...
        fprintf(off_Tracer->fp, "%d: instance[%d]_frame[%d] [%s][%s]:%s:%s",
                                off_Tracer->index,
                                off_Tracer->instanceId,
                                gframeId[off_Tracer->instanceId],
                                off_Tracer->name,
                                log_type,
                                func_name,
                                buffer);
